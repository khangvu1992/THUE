...
@Service
public class SeawayHouseQueueManager {
    ...
    private final Map<String, SeawayHouseContext> fileContexts = new ConcurrentHashMap<>();

    public void createContext(String fileId, int queueCapacity, String fileName) {
        BlockingQueue<List<SeawayHouseBillEntity>> queue = new ArrayBlockingQueue<>(queueCapacity);
        fileContexts.put(fileId, new SeawayHouseContext(queue, fileName));
    }

    public BlockingQueue<List<SeawayHouseBillEntity>> getQueue(String fileId) {
        return fileContexts.get(fileId).getQueue();
    }

    public void incrementProcessed(String fileId, int count) {
        SeawayHouseContext context = fileContexts.get(fileId);
        if (context != null) {
    ...
    public void logWaitingFiles() {
        ...
        List<SeawayHouseContext> processingFiles = fileContexts.values().stream()
                .filter(SeawayHouseContext::isStillProcessing)
                .collect(Collectors.toList());

        List<SeawayHouseContext> errorFiles = fileContexts.values().stream()
                .filter(SeawayHouseContext::hasError)
                .collect(Collectors.toList());

        if (!processingFiles.isEmpty()) {
            System.out.println("⏳ Các file đang xử lý:");
            for (SeawayHouseContext ctx : processingFiles) {
        ...
        if (!errorFiles.isEmpty()) {
            ...
            for (SeawayHouseContext ctx : errorFiles) {
                System.out.println("🚨 " + ctx.getFileName() + " - " );
            }
        }
