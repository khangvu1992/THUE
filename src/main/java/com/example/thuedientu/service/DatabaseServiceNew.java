...
public class DatabaseServiceNew {
    ...
    public DatabaseServiceNew(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    public void importDatabaseAsync(File file, HashFile hashFile) {
        String fileId = hashFile.getFileHash();
        downloadManager.enqueue(fileId, (Runnable) () -> importInternal(file, hashFile));
    }

    public void importInternal(File file, HashFile hashFile) {
        createTable();

        String fileId = hashFile.getFileHash();
        String fileName = hashFile.getFilename();
        progressTracker.startTracking(fileId, fileName);

        BlockingQueue<List<EnityExcelJDBC>> queue = new ArrayBlockingQueue<>(50);
        AtomicBoolean readingDone = new AtomicBoolean(false);

        Thread loggerThread = new Thread(() -> {
            while (!readingDone.get() || !queue.isEmpty()) {
        ...
        // 🧵 Tạo các worker thread
        List<Thread> workers = new ArrayList<>();
        for (int i = 0; i < WORKER_COUNT; i++) {
            Thread worker = new Thread(() -> workerWriteToDb(queue, readingDone, fileId), "worker-" + i);
            worker.start();
            workers.add(worker);
        }
